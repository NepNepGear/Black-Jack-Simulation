...
def blackjack_game(infinite,policy):
    ...
    while hand_value(your_Hand) <= 21:
        ...
        if policy == 1 and hand_value(your_Hand) >= 17:
            break
        # Policy 2: Conditions for a more nuanced approach
        elif policy == 2:
            # If the hand is "hard" (no Ace counted as 11) and the value is 17 or more, stick
            if hand_value(your_Hand) >= 17 and is_hard_hand(your_Hand):
                break
            # Also stick if the hand value exactly equals 21
